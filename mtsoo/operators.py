...
def multiple_crossover(pl, bl):
    par = len(pl)
    cl = deepcopy(pl)
    for i in range(par):
        cl[i] = pl[i] + bl[i]*(pl[(i+1) % par] - pl[(i+2) % par])
...
def variable_swap_mul(pl, probswap):
    # incomplete
    L = len(pl)
    D = pl[0].shape[0]
    swap_indicator = np.random.rand(D) <= probswap
    cl = deepcopy(pl)
    for i in range(0, L):
        cl[i][np.where(swap_indicator)] = pl[(i+1) %
                                             L][np.where(swap_indicator)]
...
def get_subpops(population, skill_factor, N):
    # Get subpop of each skill factor, For each skill factor, collect individuals in population of it.
...
def learn_models(subpops):
    ...
    for k in range(K):
        subpop = subpops[k]
        models.append(learn_model(subpop, D))
    return models


def learn_model(subpop, D):
