...
def multiple_crossover(p1, p2, p3, b1, b2, b3):
    c1 = p1 + b1*(p2 - p3)
    c2 = p2 + b2*(p3 - p1)
    c3 = p3 + b3*(p1 - p2)
...
def variable_swap(p1, p2, p3, probswap):
    D = p1.shape[0]
    ...
    c2[np.where(swap_indicator)] = p3[np.where(swap_indicator)]
    c3[np.where(swap_indicator)] = p1[np.where(swap_indicator)]
    return c1, c2, c3
